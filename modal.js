...
<div>
  <table class="note">
    ...
    function modalTableClick(ev) {
        if (!ev.target) return;
        var cellId = ev.target.getAttribute("data");
        if (!cellId) return;
        cellId = parseInt(cellId, 10);
        if (selectedItem != cellId) modalCallback(cellId);
        $("#myModal").modal("hide");
    }
   ...
   <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
         <div class="modal-header">
            <h5 class="modal-title">Incomplete repair factors: Fractionated irradiation (<i>H<sub>m</sub></i> factors)</h5>
    ...
    function selectRepairFactor(m, selected, callback) {
